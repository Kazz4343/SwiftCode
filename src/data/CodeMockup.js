export const codeSamples = {
  "App.jsx": 
   `import { useState } from "react";
    import { SwiftCode } from "@swiftcode/ai";

    function App() {
    const [code, setCode] = useState("");

    const handleAICompletion = async () => {
        const suggestion = await SwiftCode.complete(code);
        setCode(suggestion);
    };
    ...
    const handleAISuggestion = async () => {
        const suggestion = await SwiftCode.suggest("hero component");
        return suggestion;
    };
    ...
    const handleSearch = async () => {
        const results = await SwiftCode.search(searchQuery);
        return results;
    };
